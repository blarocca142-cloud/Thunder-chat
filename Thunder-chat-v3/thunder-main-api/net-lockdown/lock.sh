#!/usr/bin/env bash
# Blocks ALL outbound network traffic from the ollama service account.
# Ollama needs zero internet access to do inference - only to pull a
# NEW model. Run unlock.sh before pulling a model, then lock.sh again after.
set -euo pipefail
...
# Reject everything else the ollama process tries to send out.
...
echo "Locked. The ollama process (uid $OLLAMA_UID) can no longer reach the internet."
echo "Verify with: sudo iptables -L OUTPUT -n -v | grep ollama"
