...
def test_video_hook_runs_in_background(tmp_path):
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            payload = json.dumps({"video_url": "/media/hooked.mp4", "message": "clip ready"}).encode()
    ...
    try:
        ...
        assert latest["video_url"] == "/media/hooked.mp4"
        assert latest["stub"] is False
        assert "ready" in (latest.get("message") or "").lower()
    finally:
        release.set()
        creative.VIDEO_HOOK = prev
        httpd.shutdown()
